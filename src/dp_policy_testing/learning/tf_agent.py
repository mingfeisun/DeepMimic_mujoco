    ...
    def _build_normalizers(self):
        with self.sess.as_default(), self.graph.as_default(), tf.variable_scope(self.tf_scope):
            with tf.variable_scope(self.RESOURCE_SCOPE):
                ...
                self.s_norm.set_mean_std(np.zeros(self.get_state_size()), np.ones(self.get_state_size()))
                
                self.g_norm = TFNormalizer(self.sess, 'g_norm', self.get_goal_size())
                self.g_norm.set_mean_std(np.zeros(self.get_goal_size()), np.ones(self.get_goal_size()))

                self.a_norm = TFNormalizer(self.sess, 'a_norm', self.get_action_size())
                self.a_norm.set_mean_std(np.zeros(self.get_action_size()), np.ones(self.get_action_size()))
        return
